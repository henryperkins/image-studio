// Enhanced Vision Analysis Component with comprehensive controls
...
export default function EnhancedVisionAnalysis({
  ...
  const [displayMode, setDisplayMode] = useState<'structured' | 'prompt' | 'accessibility'>('structured');
  ...
  const runAnalysis = useCallback(async () => {
    ...
    try {
      ...
      const processingWarnings = analysisResult.metadata.processing_notes.filter(note => 
        note.includes('warning') || note.includes('advisory') || note.includes('redacted')
      );
      if (processingWarnings.length > 0) {
        setWarnings(prev => [...prev, ...processingWarnings]);
      }

      // Auto-generate prompt for Sora mode
      if (mode === 'sora' && onPromptGenerated) {
        onPromptGenerated(analysisResult.generation_guidance.suggested_prompt);
      }

      showToast(
        `Analysis completed with ${analysisResult.metadata.confidence} confidence`,
        'success'
      );
  ...
  }, [selectedIds, params, mode, onPromptGenerated, showToast]);
  ...
  const quickSoraAnalysis = async () => {
    ...
    showToast('Sora prompt generated!', 'success');
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button
            className="btn btn-sm text-xs"
            onClick={quickAccessibilityCheck}
            disabled={analyzing}
          >
            Accessibility Check
          </button>
      ...
      <div className="flex flex-wrap gap-2">
        <button
          className={`btn flex-1 min-w-[200px] ${analyzing ? 'pulse' : ''}`}
          onClick={runAnalysis}
          disabled={analyzing}
        >
          {analyzing ? (
          ...
          {(result || accessibilityResult) && (
            <div className="border border-neutral-700 rounded-lg">
              ...
              <div className="p-4">
                ...
                {displayMode === 'prompt' && result && <PromptDisplay result={result} onPromptGenerated={onPromptGenerated} />}
                {displayMode === 'accessibility' && accessibilityResult && <AccessibilityDisplay result={accessibilityResult} />}
              </div>
            </div>
          )}
        </div>
    ...
    <div className="space-y-4">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {result.content.primary_subjects.map((subject, index) => (
              <span
                key={index}
                className="px-2 py-1 bg-blue-600/20 text-blue-300 text-sm rounded-md"
              >
                {subject}
              </span>
            ))}
      ...
      <div>
        ...
        <div className="bg-neutral-800/50 rounded-lg p-3 space-y-2 text-sm">
          <div><strong>Composition:</strong> {result.content.visual_elements.composition}</div>
          <div><strong>Lighting:</strong> {result.content.visual_elements.lighting}</div>
          <div><strong>Style:</strong> {result.content.visual_elements.style}</div>
          <div><strong>Mood:</strong> {result.content.visual_elements.mood}</div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex gap-1">
                {result.content.visual_elements.colors.map((color, index) => (
                  <span
                    key={index}
                    className="px-2 py-1 bg-neutral-700 text-xs rounded-md"
                  >
                    {color}
                  </span>
                ))}
...
function PromptDisplay({ result, onPromptGenerated }: { 
  ...
  const copyPrompt = () => {
    navigator.clipboard.writeText(result.generation_guidance.suggested_prompt);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          {onPromptGenerated && (
            <button
              className="btn btn-sm text-xs bg-blue-600 hover:bg-blue-500"
              onClick={() => onPromptGenerated(result.generation_guidance.suggested_prompt)}
            >
              Use This Prompt
            </button>
          )}
      ...
      <div className="bg-neutral-800/50 rounded-lg p-4">
        <p className="text-sm whitespace-pre-wrap">{result.generation_guidance.suggested_prompt}</p>
      </div>

      {/* Style Keywords */}
      {result.generation_guidance.style_keywords.length > 0 && (
        <div>
          <h5 className="font-medium mb-2">Style Keywords</h5>
          <div className="flex flex-wrap gap-2">
            {result.generation_guidance.style_keywords.map((keyword, index) => (
              <span
                key={index}
                className="px-2 py-1 bg-purple-600/20 text-purple-300 text-sm rounded-md"
              >
                {keyword}
              </span>
            ))}
      ...
      <div>
        <h5 className="font-medium mb-2">Technical Parameters</h5>
        <div className="bg-neutral-800/50 rounded-lg p-3 text-sm space-y-1">
          <div className="flex justify-between">
            <span>Aspect Ratio:</span>
            <span className="font-medium">{result.generation_guidance.technical_parameters.aspect_ratio}</span>
          </div>
          <div className="flex justify-between">
            <span>Recommended Model:</span>
            <span className="font-medium">{result.generation_guidance.technical_parameters.recommended_model}</span>
          </div>
          <div className="flex justify-between">
            <span>Complexity Score:</span>
            <span className="font-medium">{result.generation_guidance.technical_parameters.complexity_score}/10</span>
          </div>
        </div>
      </div>
    </div>
      ...
      <div>
        ...
        <div className="bg-neutral-800/50 rounded-lg p-3">
          ...
          <div className="text-xs text-neutral-400 mt-2">
            {result.alt_text.length}/125 characters
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="bg-neutral-800/50 rounded-lg p-3 space-y-2 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className={`font-medium ${result.color_accessibility.relies_on_color ? 'text-yellow-400' : 'text-green-400'}`}>
              {result.color_accessibility.relies_on_color ? 'Yes' : 'No'}
            </span>
          </div>
          <div className="flex justify-between">
            <span>Color Blind Safe:</span>
            <span className={`font-medium ${result.color_accessibility.color_blind_safe ? 'text-green-400' : 'text-red-400'}`}>
